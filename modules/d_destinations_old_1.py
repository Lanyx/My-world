...
# GET THE LAND USAGE.
    ...
    for child in aChildren[1:]:     # Start for loop from second element
        ...
        xParam = {"my_id": child}             # The parent is common to all
        xRestr = {"_id":0}
        dQuery = cDest.find(xParam, xRestr)
        ...
        # Convert the area.
        dBad_area = misc.calc_area(fA, dChild["aMap"]["fScale"])
        if dBad_area == None: return None
        # There is some inconsistency in the formula.
        dGood_area = {"val": dBad_area["qty"], "uom": dBad_area["uom"]}

        # How many work-places are there?
        sTxt = "Please enter number of workplaces in the area:"
        ...
        for iHit_cnt in range(iNo_of_hits):
            ...
            sTxt = sTxt.format(iHit_cnt+1, iNo_of_hits)
            print(sTxt)
            # Wire up to option 'w' in the main menu.
            xOut = add_workplace(ccTremb, sGeo_code, default_names=True)
            if xOut == None: return None
    ...
    # balance the Municipality

# WORK ON THE CAPITAL:
    # Balance county
    # Ask for industry and offices
    # Automate the rest
...
# SUPERVISOR DEMOGRAPHIC CALCULATION
    ...
    return aDemogfx
#-------------------------------------------------------------------------------
def add_workplace(ccTremb, sGeo_code=None, default_names=False):
...
# START TO PROCESS
    ...
    if dWkp["default"] != "" or dWkp["default"] != None:
        ...
        if default_names == False:      # Fully manual option
    ...
    # EXACT FOOTPRINT:
    if default_names == False:      # Manual method
...
#def qGen_work_demand(dWkp, dFarm_footprint):
    """ Method is a seperation from the selection and the processing. """
